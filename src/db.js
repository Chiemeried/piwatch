...
export async function addWallet(label, address, telegramToken, chatId) {
  const result = await pool.query(
    `INSERT INTO wallets (label, address, telegram_token, chat_id)
...
export async function toggleWallet(id) {
  const result = await pool.query(
    `UPDATE wallets SET active = NOT active WHERE id = $1 RETURNING *`,
    [id]
  )
  return result.rows[0]
}
...
export async function updateLastTx(address, txId) {
  await pool.query(
    'UPDATE wallets SET last_tx_id = $1 WHERE address = $2',
    [txId, address]
  )
}
...
export async function saveTx(walletAddress, txId, type, amount, from, to) {
  try {
    await pool.query(
      `INSERT INTO transactions (wallet_address, tx_id, type, amount, from_address, to_address)
       VALUES ($1, $2, $3, $4, $5, $6)`,
...
export async function getTransactions(limit = 50) {
  const result = await pool.query(
    `SELECT t.*, w.label FROM transactions t
